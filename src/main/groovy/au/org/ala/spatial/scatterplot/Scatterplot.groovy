...
//        log.debug ("Get layers ....");
            ...
            for (int i = 0; i < layercount - 1; i++) {
                for (int j = i + 1; j < layercount; j++) {
                    ...
                    try {
                        ...
                        log.error("Failed to create a scatterplot: " + e.getMessage())
                    }
                }
            }
    ...
    static String translateFieldForSolr(String facetName) {
        ...
        if ("occurrence_year_individual" == facetName) {
            facetName = "occurrence_year"
        }
        if ("occurrence_year_decade" == facetName) {
            facetName = "occurrence_year"
        }
        return facetName
    }
    ...
    static Scatterplot load(File file, GridCutterService gridCutterService, LayerIntersectService layerIntersectService) {
        try {
            ...
            log.error("error reading scatterplot from disk", e)
        }
        return null
    }
    ...
    void buildScatterplot() {
        ...
        } else annotation_change = old_annotation != null || scatterplotStyleDTO.getSelection() != null

        //identify layer changes
        boolean layer_change = scatterplotDataDTO.getLayer1() == null || scatterplotDataDTO.getLayer1() != scatterplotDTO.getLayers()[col1]
                || scatterplotDataDTO.getLayer2() == null || scatterplotDataDTO.getLayer2() != scatterplotDTO.getLayers()[col2]
    ...
    void renderScatterplot(String filename_part) {
        ...
        try {
            ...
            try {
                ...
                log.error("error writing scatterplot png", e)
            }

        } catch (Exception e) {
            log.error("error producing scatterplot", e)
        }
    }
    ...
    String getCSV() {
        ...
        for (int i = 0; i < series.length; i++) {
            ...
            if (scatterplotDataDTO.getGridData() != null) {
                int pos1 = Arrays.binarySearch(scatterplotDataDTO.getGridCutoffs()[0], (float) d[i][0])
                if (pos1 < 0) {
                    pos1 = (pos1 * -1) - 1
                }
                int pos2 = Arrays.binarySearch(scatterplotDataDTO.getGridCutoffs()[1], (float) d[i][1])
                if (pos2 < 0) {
                    pos2 = (pos2 * -1) - 1
                }
    ...
    void createBlockPlot(float min1, float max1, float min2, float max2) {
        ...
        if (cutDataPath == null) {  //only need to cut once if rebuilding
            ...
            cutDataPath = gridCutterService.cut2(layers, resolution, region, envelope, null, types, scatterplotDTO.getLayers())
        }
    ...
    static int getPos(float d, float[] cutoffs) {
    ...
    private void createDataset(boolean foreground) {
        ...
        for (int i = 0; i < seriesNames.length; i++) {
            ...
            for (int j = 0; j < d.length; j++) {
                if (seriesNames.length == 1 || series[j] == seriesNames[i]) {
    ...
    private void createAADataset() {
        ...
        if (aaDataset != null
                ...
                && scatterplotStyleDTO.getPrevHighlightWkt() != scatterplotStyleDTO.getHighlightWkt())) {
            return
        }
    ...
    XYShapeRenderer getRenderer(LegendObject legend, int[] datasetColours, String[] seriesNames, double[] seriesValues) {
        ...
        if (datasetColours != null) {
            ...
            for (int i = 0; i < datasetColours.length; i++) {
                ...
                if (seriesNames[i] == NUMBER_SERIES) {
    ...
    private void resample(boolean foreground) {
        ...
        boolean resampleColourMode = scatterplotStyleDTO.getColourMode() != scatterplotDataDTO.getColourMode()
        ...
        try {
            ...
            if (points == null) {   //get points
                ...
                for (int i = 1; i < csv.size(); i++) {
                    ...
                    ids[(int)(pos / 2)] = csv.get(i)[idColumn]
                    pos += 2
                }
            ...
            if (scatterplotStyleDTO.getColourMode() == "-1" || !foreground) {
            ...
            if (foreground) {
                log.error("failed to sample points: q=" + scatterplotDTO.getForegroundOccurrencesQs() + ", bs=" + scatterplotDTO.getForegroundOccurrencesBs() + ", fields=" + fields, e)
            } else {
                log.error("failed to sample points: q=" + scatterplotDTO.getBackgroundOccurrencesQs() + ", bs=" + scatterplotDTO.getBackgroundOccurrencesBs() + ", fields=" + fields, e)
            }
    ...
    private String[] sampleBiocacheSeries() {
        try {
            ...
            log.error("cannot get colourmode for biocache series", e)
        }
    ...
    private static int findInArray(String lookFor, String[] array) {
    ...
    ScatterplotDTO getScatterplotDTO() {
    ...
    ScatterplotStyleDTO getScatterplotStyleDTO() {
        return scatterplotStyleDTO
    }

    ScatterplotDataDTO getScatterplotDataDTO() {
        return scatterplotDataDTO
    }

    void annotatePixelBox(Integer minx, Integer miny, Integer maxx, Integer maxy) {
        try {
            ...
            log.error("failed to apply annotation")
        }
    }

    void reStyle(ScatterplotStyleDTO style, boolean colourModeUpdate, boolean redUpdate, boolean blueUpdate
    ...
    private void makeHtml() {
        ...
        log.debug("ScatterplotList file: " + dir + "scatterplot.html")
        log.debug("ScatterplotList output: " + sb)
    }

    private static void makeReadme(String pth) {
        try {
    ...
    LegendObject getLegend(String colourmode) {
        if (legends.get(colourmode) != null) {
            ...
            try {
                ...
                //test for exceptions
                if (!colourmode.contains(",") && (colourmode == "uncertainty" || colourmode == "decade" || colourmode == "occurrence_year" || colourmode == "coordinate_uncertainty")) {
                    ...
                    String newFacet = colourmode == "decade" ? "occurrence_year" : colourmode
                    sb.append(newFacet)
                    int i = 0
                    int lasti = 0
                    while (i < cutpoints.length) {
                        if (i == cutpoints.length - 1 || cutpoints[i] != cutpoints[i + 1]) {
                            if (i > 0) {
                                ...
                                if (colourmode == "occurrence_year" || colourmode == "decade")
                    ...
                    lo = newlo
                } else if (!colourmode.contains(",") && (colourmode == "occurrence_year_decade" || colourmode == "decade")) {
                ...
                log.error("error getting legend for : " + colourmode, e)
            }
        }
        return null
    }
    ...
    void save(File file) {
        ...
        try {
            ...
            new File(file.getPath() + ".dto").write(new ObjectMapper().writeValueAsString(scatterplotDTO))
            new File(file.getPath() + ".style").write( new ObjectMapper().writeValueAsString(scatterplotStyleDTO))
            new File(file.getPath() + ".data").write( new ObjectMapper().writeValueAsString(scatterplotDataDTO))
            file.write( new ObjectMapper().writeValueAsString(list))
        } catch (Exception e) {
            log.error("failed to save scatterplot data: " + file, e)
        }
    }
    ...
    void saveCsv(File file) {
        try {
            ...
            log.error("failed to save scatterplot csv: " + file, e)
        }
    }
...
class LegendFieldPaintScale implements PaintScale, Serializable {
    ...
    LegendFieldPaintScale(LegendObject legend, Color defaultColour) {
        this.legend = legend
...
class MyXYShapeRenderer extends XYShapeRenderer {
    ...
    @Override
    Paint getPaint(XYDataset dataset, int series, int item) {
