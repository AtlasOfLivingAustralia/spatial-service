...
class GetValuesSpeciesThread extends Thread {
     ...
     * @param parts
     */
    GetValuesSpeciesThread(LinkedBlockingQueue<Integer> parts) {
        this.parts = parts
    }
     ...
     * @param row
     */
    void set(CountDownLatch cdl, int partSize, BitSet[][] bsRows,
            ...
            while (true) {
                ...
                for (int i = n * partSize; i < len; i++) {
                    ...
                    if (cellcount == 0) {
                        ...
                        values[i] = (float) (thisCell.cardinality() / (float) cellcount)
                    }
                }
                cdl.countDown()
            }
