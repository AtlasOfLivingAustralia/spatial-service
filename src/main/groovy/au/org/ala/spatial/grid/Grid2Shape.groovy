...
//@CompileStatic
 class Grid2Shape {
    ...
    static  String grid2Wkt(BitSet data, int nrows, int ncols, double minx, double miny, double resx, double resy) {
    ...
    static  HashMap grid2WktIndexed(BitSet data, int nrows, int ncols, double minx, double miny, double resx, double resy, int startKey) {
    ...
    static  void streamGrid2Wkt(OutputStream os, BitSet data, int nrows, int ncols, double minx, double miny, double resx, double resy) throws IOException {
        int gid = 0

        os.write("MULTIPOLYGON(".bytes)
        gid = getPolygonsWkt(os, gid, data, nrows, ncols, resx, resy, minx, miny)
        os.write(")".bytes)
    }

    static  void streamGrid2WktIndexed(OutputStream os, BitSet data, int nrows, int ncols, double minx, double miny, double resx, double resy, int[] map, StringBuilder index, int startKey) throws IOException {
        int characterPos = 0
    ...
    static  MultiPolygon grid2Multipolygon(BitSet data, int nrows, int ncols, double minx, double miny, double resx, double resy) {
        int gid = 0
    ...
    static  String grid2Wkt(float[] data, double minValue, double maxValue, int nrows, int ncols, double minx, double miny, double resx, double resy) {
        return (String) grid2Object(true, data, minValue, maxValue, nrows, ncols, minx, miny, resx, resy, null)
    }

    static  HashMap grid2WktIndexed(float[] data, double minValue, double maxValue, Set keys, int nrows, int ncols, double minx, double miny, double resx, double resy, int[] map) {
    ...
    static  void streamGrid2Wkt(OutputStream os, float[] data, double minValue, double maxValue, int nrows, int ncols, double minx, double miny, double resx, double resy) {
    ...
    static  MultiPolygon grid2Multipolygon(float[] data, double minValue, double maxValue, int nrows, int ncols, double minx, double miny, double resx, double resy) {
    ...
    static  int getPolygons(ArrayList<Polygon> output, int gid, BitSet data, int nrows, int ncols, double xres, double yres, double minx, double miny) {
        int[] image = new int[data.size()]
        byte[] edges = new byte[data.size()]
                    ...
                    if (j > 0 && image[pos - 1] > 0) {
                        ...
                        if (!joinedWithNewGroup
                            ...
                            if (j > 0 && i > 0 && image[pos - ncols] == image[pos]) {
                                ...
                                edges[pos] = (byte) (edges[pos] | bEdge)
                            }
                        ...
                        if (j == 0 || image[pos - 1] != image[pos]) {
                            edges[pos] = (byte) (edges[pos] | lEdge)
                        }
    ...
    static private void moveNext(Pos current, byte[] edges, int[] image, int nrows, int ncols) {
     ...
     static int getArrayMax(int[] wktMap) {
        int max = wktMap[0]
     ...
     static int getArrayMin(int[] wktMap) {
        int min = wktMap[0]
         for (int i = 1; i < wktMap.length; i++) {
