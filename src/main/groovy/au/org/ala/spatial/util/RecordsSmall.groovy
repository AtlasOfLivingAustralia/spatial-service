...
class RecordsSmall {
    ...
    RecordsSmall(String dir) throws IOException {
        ...
        if (!smallFile.exists() && new File(filename + "records.csv").exists()) {
            try {
                ...
                log.error("failed to make small records files", e)
            }
        }
        ...
        if (smallFile.exists()) {
            try {
                ...
                maxPoints = new File(filename + "records.csv.small.pointsToSpecies").length() / 4 as long
                ...
                log.error("failed to open small records file", e)
            }
        }
    }

    private static void makeSmallFile(String filename) throws Exception {
        ...
        while ((rawline = br.readLine()) != null) {
            ...
            if (p1 >= 0 && p2 >= 0) {
                ...
                if (row == 0) {
                    ...
                    log.debug("header: " + header[0] + "," + header[1] + "," + header[2])
                    ...
                    try {
                        ...
                        log.error("failed to read records.csv row: " + row, e)
                    }
                }
        ...
        new File(filename + "records.csv.small.speciesCount").write(String.valueOf(lsidMap.size()))
    }
    ...
    private void makeUniquePoints() throws Exception {
        ...
        Coord[] p = new Coord[(int)(allPoints.length / 2)]
        for (int i = 0; i < allPoints.length; i += 2) {
            p[(int)(i / 2)] = new Coord(allPoints[i], allPoints[i + 1], i / 2 as int)
        }
        allPoints = null //make available to GC
        Arrays.sort(p, new Comparator<Coord>() {
            int compare(Coord o1, Coord o2) {
    ...
    int getSpeciesNumber(int pos) throws Exception {
    ...
    double[] getPointUnsafe(int pos) throws Exception {
    ...
    double[] getPointsAll() throws Exception {
        return getAllDouble(filename + "records.csv.small.points")
    }

    private static double[] getAllDouble(String file) throws Exception {
        File f = new File(file)

        int size = f.length() / 8 as int
    ...
    void close() throws Exception {
        try {
            ...
            log.error("failed to close records.small", e)
        }
    }

    // this is a reset for the 'getNext...()' functions
    void resetNextFunctions() throws Exception {
        pointsDis.close()
        pointsToSpeciesDis.close()
    ...
    int getNextSpecies() throws Exception {
        return pointsToSpeciesDis.readInt()
    }

    // for sequential reads, longitude then latitude
    double getNextCoordinate() throws Exception {
        return pointsDis.readDouble()
    }

    double getLongitude(int pos) throws Exception {
        return getPoint(pos)[1]
    }

    double getLatitude(int pos) throws Exception {
    ...
    private class Coord {
        ...
        Coord(double latitude, double longitude, int pos) {
