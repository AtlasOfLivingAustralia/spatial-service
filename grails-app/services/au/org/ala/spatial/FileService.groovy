...
class FileService {
    ...
    def info(String name) {
        ...
        if (list != null && list.size() > 0) {
            ...
            list.each { File file ->
                if (file.isDirectory()) {
                    String relativePath = file.getPath().substring(spatialConfig.data.dir.toString().length(), file.getPath().length())
     ...
     * @return
     */
    def zip(OutputStream outputStream, List<File> files, String prefix, compress) {
    ...
    def zipEntryName(String path, String prefix) {
        ...
        int length = spatialConfig.data.dir.toString().length()
